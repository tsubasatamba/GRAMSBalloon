...
bool SlowADCIO::getData(int channel, uint16_t& adc, double& voltage, int num_trials/*=2*/)
{
  int rslt = 0;
  std::vector<char> buf;
  rslt = readReg(channel, buf, num_trials);
  if (rslt < 0) {
  ...
  for (int i=0; i<num_trials; i++) {
    if (i==0) continue;
    uint16_t value = static_cast<uint16_t>((static_cast<uint16_t>(buf[i*2]) << 8) + (static_cast<uint16_t>(buf[i*2+1])));
...
int SlowADCIO::readReg(int channel, std::vector<char>& buf, int num_trials/* =2 */)
{
  if (channel<0 || channel>=SLOW_ADC_CHANNEL_MAX) {
    return -1;
  }

  const int byte = num_trials*2;
