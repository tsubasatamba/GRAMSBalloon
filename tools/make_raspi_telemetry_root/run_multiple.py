...
def run(runIDs: List[int], output_root: str):
    ...
    print(f"{exe_file} {temp_filelist} {output_root}")
...
if __name__ == "__main__":
    if len(sys.argv) <= 2:
        raise ValueError("USAGE: ./run_multiple.py output_file_header runI_D1 runID_2 runID_3 ...")
    output_file = sys.argv[1]
    run_ids = sys.argv[2:]
    for id in run_ids:
        output_file_with_id = output_file + f"{id}.root"
        run([int(id),], output_file_with_id)
