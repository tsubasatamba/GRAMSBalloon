...
ssize_t SocketCommunication::send(const T &data) {
  for (int i = 0; i < numTrial_; i++) {
    const auto send_result = ::send(socket_, data, sizeof(T), 0); // TODO: this depends on telemetry definition.
    if (send_result == -1) {
...
inline ssize_t SocketCommunication::send(const std::vector<uint8_t> &data) {
  auto ret = ::send(socket_, data.data(), data.size(), 0);
  if (ret < 0) {
    std::cerr << "Error in SocketCommunication::send: Sending data failed. error code = " << errno << "(" << strerror(errno) << ")" << std::endl;
    failed_ = true;
  }
  return ret;
}
...
ssize_t SocketCommunication::receive(T &data) {
  auto ret = ::recv(socket_, &data, sizeof(T), 0);
  if (ret < 0) {
